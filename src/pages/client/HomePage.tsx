...
export default function HomePage() {
  ...
  const Field = ({
    ...
    <div className="flex flex-col gap-1.5 flex-1">
      <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider px-1">{label}</span>
      <div className="flex items-center gap-3 px-4 py-4 bg-slate-50 border border-slate-100
          ...
          <nav className="hidden md:flex items-center gap-10">
            {['Trang chủ', 'Tuyến đường', 'Lịch trình', 'Hỗ trợ'].map((l, i) => (
              <button 
                ...
                {l}
                {i === 0 && <span className="absolute bottom-0 left-0 w-full h-0.5 bg-brand-primary rounded-full" />}
              </button>
            ))}
          </nav>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col items-center text-center mb-16 px-4">
            ...
            <h1 className="text-5xl lg:text-7xl font-black text-slate-900 leading-[1.1] mb-6">
              Khám phá hành trình mới<br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-primary to-brand-accent">Cùng Go Routex</span>
            </h1>
            
            <p className="text-slate-500 text-lg lg:text-xl max-w-2xl leading-relaxed">
              Dịch vụ đặt vé xe liên tỉnh hiện đại nhất. <br className="hidden md:block" /> 
              ...
              <div className="flex items-center gap-8 px-8 py-3 bg-slate-50/50 rounded-b-[22px] border-t border-slate-50">
                   ...
                   {(['one-way', 'round-trip'] as const).map(t => (
                    <label key={t} className="flex items-center gap-2 cursor-pointer group">
                      <input 
                        ...
                        className="hidden"
                      />
                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${tripType === t ? 'border-brand-primary' : 'border-slate-300 group-hover:border-slate-400'}`}>
                        {tripType === t && <div className="w-2.5 h-2.5 bg-brand-primary rounded-full" />}
                      </div>
                      <span className={`text-sm font-bold ${tripType === t ? 'text-slate-900' : 'text-slate-500 hover:text-slate-700'}`}>
                ...
                {STATS.map(s => (
                  <div key={s.label} className="flex items-center gap-2">
                    <span className="text-brand-primary font-black">{s.value}</span>
      ...
      <main className="max-w-7xl mx-auto px-6 py-12 grid grid-cols-1 lg:grid-cols-4 gap-12">
        ...
        <div className="lg:col-span-3 space-y-16">
          ...
          <section>
            <div className="flex items-end justify-between mb-8">
              <div>
                <h2 className="text-3xl font-black text-slate-900">Tuyến đường phổ biến</h2>
                <p className="text-slate-500 mt-2">Được nhiều người dùng lựa chọn nhất</p>
              </div>
                ...
                >
                  ...
                  <div className="flex items-center justify-between mb-6 relative z-10">
                    <div className="bg-slate-50 px-4 py-1.5 rounded-full">
                      <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Premium trip</span>
                    </div>
                    <span className="text-2xl font-black text-slate-900 group-hover:text-brand-primary transition-colors">{r.price}</span>
                  </div>
          ...
          <section className="bg-white rounded-[3rem] p-12 border border-slate-100 shadow-xl shadow-slate-100">
            <h2 className="text-3xl font-black text-slate-900 mb-10 text-center">Tại sao nên chọn Go Routex?</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
              {[
                { icon: Clock,      title: 'Đúng giờ', desc: 'Cam kết khởi hành đúng lịch trình, tối ưu thời gian của bạn.' },
                { icon: Armchair,   title: 'Ghế ngồi VIP',   desc: 'Hệ thống ghế ngồi/giường nằm cao cấp, thoải mái nhất.' },
                { icon: TrendingUp, title: 'Giá tốt nhất', desc: 'Luôn đảm bảo mức giá cạnh tranh nhất thị trường.' },
                { icon: CheckCircle2, title: 'Xác nhận ngay', desc: 'Có vé ngay sau khi thanh toán, tiện lợi 24/7.' },
        ...
        <aside className="space-y-8">
          ...
          <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white relative overflow-hidden shadow-2xl shadow-slate-200">
            ...
            <h3 className="font-black text-xl mb-8 relative z-10">Quy trình đặt vé</h3>
            <ol className="space-y-8 relative z-10">
              {[
                ...
                <li key={step} className="flex items-center gap-5">
                  <span className="text-xs font-black text-brand-primary bg-brand-primary/10 w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 border border-brand-primary/20">
                    {step}
                  </span>
                  <p className="text-white/80 text-sm font-bold leading-snug">{text}</p>
                </li>
              ))}
            </ol>
          </div>
          ...
          <div className="lg:hidden grid grid-cols-2 gap-4">
               {STATS.map(s => (
                  <div key={s.label} className="bg-white p-6 rounded-[2rem] border border-slate-100 text-center">
                    <div className="text-brand-primary text-2xl font-black">{s.value}</div>
                    <div className="text-slate-400 text-[10px] font-black uppercase tracking-widest mt-1">{s.label}</div>
                  </div>
                ))}
          </div>
          ...
          {!isLoggedIn ? (
            <div className="bg-gradient-to-br from-brand-primary to-brand-accent rounded-[2.5rem] p-8 text-white shadow-xl shadow-brand-primary/20">
              <h3 className="font-black text-xl mb-3">Tham gia Go Routex ngay!</h3>
              <p className="text-white/80 text-sm mb-8 leading-relaxed">
                Tích điểm mỗi chuyến đi, nhận hàng ngàn ưu đãi độc quyền dành riêng cho bạn.
              </p>
              <button
            ...
            <div className="bg-white border-2 border-brand-primary/10 rounded-[2.5rem] p-8 text-center">
              ...
              <h3 className="font-black text-slate-900 text-lg mb-2">Xem lịch sử đặt vé</h3>
              <p className="text-slate-500 text-xs mb-6">Quản lý các chuyến đi của bạn dễ dàng hơn bao giờ hết.</p>
              <button className="text-brand-primary font-black text-sm hover:underline">Khám phá ngay</button>
            </div>
          )}
        </aside>
      </main>
      ...
      <footer className="mt-24 bg-white border-t border-slate-100">
        <div className="max-w-7xl mx-auto px-6 py-16">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
            <div className="md:col-span-1">
              ...
              <p className="text-slate-500 text-sm leading-relaxed">
                Hệ thống đặt vé xe công nghệ hàng đầu Việt Nam. Mang lại trải nghiệm di chuyển hiện đại, an toàn và thông minh.
              </p>
            </div>
            <div>
              <h4 className="font-black text-slate-900 mb-6 font-bold uppercase tracking-widest text-xs">Về chúng tôi</h4>
              <ul className="space-y-4">
                 {['Tuyển dụng', 'Tin tức', 'Liên hệ', 'Quy định chung'].map(l => (
                  <li key={l}><a href="#" className="text-slate-500 text-sm hover:text-brand-primary transition-colors">{l}</a></li>
                 ))}
              </ul>
            </div>
            <div>
              <h4 className="font-black text-slate-900 mb-6 font-bold uppercase tracking-widest text-xs">Hỗ trợ khách hàng</h4>
              <ul className="space-y-4">
                 {['Chính sách bảo mật', 'Điều khoản sử dụng', 'Hướng dẫn thanh toán', 'Câu hỏi thường gặp'].map(l => (
                  <li key={l}><a href="#" className="text-slate-500 text-sm hover:text-brand-primary transition-colors">{l}</a></li>
                 ))}
              </ul>
            </div>
            <div>
              <h4 className="font-black text-slate-900 mb-6 font-bold uppercase tracking-widest text-xs">Tải ứng dụng</h4>
              <div className="flex flex-col gap-3">
                 <div className="bg-slate-900 p-4 rounded-xl flex items-center gap-4 cursor-pointer hover:bg-slate-800 transition-colors">
                    ...
                    <div>
                       <p className="text-[10px] text-white/50 font-bold leading-none mb-1">Download on the</p>
                 ...
                 <div className="bg-slate-900 p-4 rounded-xl flex items-center gap-4 cursor-pointer hover:bg-slate-800 transition-colors">
                    ...
                    <div>
                       <p className="text-[10px] text-white/50 font-bold leading-none mb-1">GET IT ON</p>
          ...
          <div className="pt-8 border-t border-slate-100 flex flex-col md:flex-row justify-between items-center gap-6">
            <span className="text-slate-400 text-sm font-medium">© 2026 Go Routex JSC. All rights reserved.</span>
            <div className="flex gap-8">
               {['Facebook', 'LinkedIn', 'Instagram'].map(s => (
                <a key={s} href="#" className="text-slate-400 hover:text-brand-primary transition-colors text-sm font-bold">{s}</a>
               ))}
            </div>
          </div>
        </div>
