...
import { ArrowLeft, Bus, MapPin, User, Phone, Mail, StickyNote, ChevronRight } from 'lucide-react'
...
const InputField = ({
  ...
  <div className="space-y-3">
    ...
    <div className="flex items-start gap-5 p-5 bg-slate-50 border-2 border-slate-50 focus-within:border-brand-primary/30 focus-within:bg-white rounded-[1.5rem] transition-all duration-300 shadow-sm focus-within:shadow-xl focus-within:shadow-brand-primary/10 group">
      <div className="w-12 h-12 rounded-[1rem] bg-white flex items-center justify-center shadow-sm flex-shrink-0 mt-0.5 border border-slate-100 group-focus-within:border-brand-primary/20 transition-all">
...
export default function BookingPage() {
  ...
  const handleConfirm = () => {
    ...
    navigate('/payment', { state: { routeData, selectedSeats, customerName, customerPhone, customerEmail, totalAmount, note } })
  }

  return (
    <div className="min-h-screen bg-[#F8FAFC] font-sans pb-64">
      {/* Header */}
      <div className="bg-brand-dark pt-12 pb-20 px-8 relative overflow-hidden"
        ...
        <div className="max-w-5xl mx-auto flex items-center justify-between relative z-10">
          ...
          <div className="text-center">
             <h1 className="text-white font-black text-2xl tracking-tight">Đặt vé</h1>
             <p className="text-slate-400 text-[10px] font-black uppercase tracking-[0.2em] mt-1 opacity-70">Nhập thông tin hành khách</p>
          </div>
          <div className="w-12" />
        </div>
      </div>
      ...
      <main className="max-w-5xl mx-auto px-8 py-12 -mt-10 grid grid-cols-1 md:grid-cols-12 gap-8">
        <div className="md:col-span-7 space-y-8">
           ...
           <div className="bg-white rounded-[2.5rem] border border-slate-100 p-10 shadow-xl shadow-slate-200/50 space-y-8">
              ...
              <InputField label="Họ và tên" icon={User} placeholder="Nhập đầy đủ họ tên" value={customerName} onChange={setCustomerName} />
              <InputField label="Số điện thoại" icon={Phone} placeholder="Nhập số điện thoại liện hệ" type="tel" value={customerPhone} onChange={setCustomerPhone} />
              <InputField label="Email" icon={Mail} placeholder="Nhập địa chỉ email nhận vé" type="email" value={customerEmail} onChange={setCustomerEmail} />
              <InputField label="Ghi chú" icon={StickyNote} placeholder="Yêu cầu đặc biệt hoặc ghi chú thêm..." value={note} onChange={setNote} multiline />
           </div>
        </div>
        ...
        <div className="md:col-span-5 space-y-8">
           ...
           <div className="bg-white rounded-[2.5rem] border border-slate-100 p-10 shadow-xl shadow-slate-200/50 relative overflow-hidden group">
              ...
              <span className="text-brand-primary text-[11px] font-black uppercase tracking-[0.2em] mb-4 bg-brand-primary/10 px-3 py-1 rounded-lg border border-brand-primary/10 inline-block">{routeData.routeCode}</span>
              <h2 className="text-3xl font-black text-slate-900 tracking-tighter mb-8">{routeData.origin} <span className="text-slate-300 mx-1">→</span> {routeData.destination}</h2>

              <div className="flex items-center gap-8 mb-8">
                <div>
                  <div className="text-2xl font-black text-slate-900 tracking-tight">{formatTimeHHmm(routeData.plannedStartTime)}</div>
                  <div className="text-xs text-slate-400 font-bold mt-1 uppercase tracking-widest">Đi</div>
                </div>
                ...
                <div className="text-right">
                  <div className="text-2xl font-black text-slate-900 tracking-tight">{formatTimeHHmm(routeData.plannedEndTime)}</div>
                  <div className="text-xs text-slate-400 font-bold mt-1 uppercase tracking-widest">Đến</div>
                </div>
              </div>

              <div className="pt-8 border-t border-slate-50 space-y-4">
                <div className="flex items-center justify-between">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Điểm đón</p>
                  <div className="flex items-center gap-2">
                    <MapPin className="w-4 h-4 text-brand-primary" />
                    <span className="font-black text-slate-900 text-sm tracking-tight">{routeData.pickupBranch || '—'}</span>
                  </div>
                </div>
                <div className="flex items-center justify-between">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Xe vận hành</p>
                  <div className="flex items-center gap-2">
                    <Bus className="w-4 h-4 text-brand-secondary" />
                    <span className="font-black text-slate-900 text-sm tracking-tight">{routeData.vehicleType || '—'} • {routeData.vehiclePlate || '—'}</span>
                  </div>
                </div>
              </div>
           </div>
           ...
           <div className="bg-white rounded-[2.5rem] border border-slate-100 p-10 shadow-xl shadow-slate-200/50">
              <h3 className="text-xl font-black text-slate-900 mb-6 tracking-tight flex items-center justify-between">
                 Vị trí ghế đã chọn
                 <span className="text-brand-primary text-sm font-black bg-brand-primary/10 px-3 py-1 rounded-lg">{selectedSeats.length} Ghế</span>
              </h3>
              <div className="flex flex-wrap gap-3">
                {selectedSeats.length === 0 ? (
                  ...
                  selectedSeats.map(seat => (
                    <span key={seat} className="px-5 py-2.5 bg-brand-primary/10 text-brand-primary font-black rounded-xl border border-brand-primary/10 text-base shadow-sm">
                      {seat}
                    </span>
                  ))
                )}
           ...
           <div className="bg-brand-dark rounded-[2.5rem] p-10 shadow-2xl shadow-brand-dark/20 text-white relative overflow-hidden group">
              ...
              <h3 className="text-xl font-black mb-8 tracking-tight">Chi tiết thanh toán</h3>
              <div className="space-y-5">
                <div className="flex justify-between items-center text-slate-400">
                  <span className="font-bold text-sm uppercase tracking-widest">Số lượng ghế</span>
                  <span className="font-black text-white text-base">x {selectedSeats.length}</span>
                </div>
                <div className="flex justify-between items-center text-slate-400">
                  <span className="font-bold text-sm uppercase tracking-widest">Đơn giá</span>
                  <span className="font-black text-white text-base">{formatVnd(unitPrice)}</span>
                </div>
                <div className="pt-6 mt-2 border-t border-white/10 flex justify-between items-center">
                  <span className="text-xl font-black text-slate-300 tracking-tight">Tổng cộng</span>
                  <span className="text-4xl font-black text-brand-primary tracking-tighter">{formatVnd(totalAmount)}</span>
                </div>
              </div>
           </div>
        </div>
      ...
      {/* Bottom Action Bar */}
      <div
        className="fixed bottom-0 left-0 right-0 px-8 py-8 border-t border-slate-100 z-50 bg-white/80 backdrop-blur-2xl"
      >
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-slate-900 font-black text-lg mt-0.5 tracking-tight">
                  {customerName.trim().length > 0 ? customerName : 'Chưa nhập tên'}
                </p>
              </div>
            </div>
            <div className="text-right">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Tổng thanh toán</p>
              <p className="text-brand-primary font-black text-2xl mt-0.5 tracking-tighter">{formatVnd(totalAmount)}</p>
            </div>
          </div>
