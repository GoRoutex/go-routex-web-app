...
const searchTargets = [
  ...
  {
    terms: ["vehicle", "vehicles", "fleet", "xe", "doi xe", "phuong tien"],
    to: "/admin/vehicles",
  },
  {
    terms: [
  ...
  {
    terms: ["location", "province", "city", "tinh", "thanh pho"],
    to: "/admin/locations",
  },
  {
    terms: ["schedule", "lich", "lich trinh", "lich chuyen"],
    to: "/admin/schedules",
  },
...
export function TopNav({
  ...
  const breadcrumbs = useMemo(() => {
    ...
    const base = (label: string) => (
      <div className="flex items-center gap-2">
        <Link
          to="/admin/dashboard"
          className="text-slate-400 font-bold hover:text-brand-primary transition-colors"
        >
          Bảng điều khiển
        </Link>
    ...
    const map: Record<string, ReactNode> = {
      ...
      "/admin/health": base("Tình trạng hệ thống"),
      "/admin/feedback": base("Phản hồi"),
    };
  ...
  const performSearch = () => {
    ...
    window.alert(
      "Không tìm thấy mục phù hợp. Hãy thử: xe, tuyến, vé, bảo trì, nhân sự, doanh thu.",
    );
  };

  return (
    <header
      className={`h-20 flex items-center justify-between px-8 border-b backdrop-blur-sm shrink-0 transition-colors duration-300 ${
        isDarkMode
    ...
    >
      <div className="flex items-center gap-6">
        ...
        <div className="flex items-center gap-2 text-[14px]">{breadcrumbs}</div>
      </div>
      ...
      <div className="flex items-center gap-6">
        ...
        <div
          className={`flex items-center gap-4 ${isDarkMode ? "text-slate-300" : "text-slate-400"}`}
        >
          <button
            type="button"
          ...
          <button
            ...
            className={`w-10 h-10 rounded-xl flex items-center justify-center cursor-pointer transition-all border border-transparent ${
              isDarkMode
                ? "hover:bg-slate-800 hover:text-brand-primary hover:border-slate-700"
                : "hover:bg-slate-50 hover:text-brand-primary hover:border-slate-100"
            }`}
        ...
        >
          ...
          <button
            ...
            title={
              isRightSidebarVisible ? "Đóng khu thông báo" : "Mở khu thông báo"
            }
          >
        ...
        <div
          className={`h-8 w-px mx-2 ${isDarkMode ? "bg-slate-800" : "bg-slate-100"}`}
        />

        <button
          type="button"
          onClick={() => navigate("/admin/profile/overview")}
          className="flex items-center gap-3 cursor-pointer group"
          title="Hồ sơ cá nhân"
        >
          <div className="text-right hidden sm:block">
            <p
              className={`text-sm font-black leading-none ${isDarkMode ? "text-slate-100" : "text-slate-900"}`}
            >
              {profile.name}
            </p>
